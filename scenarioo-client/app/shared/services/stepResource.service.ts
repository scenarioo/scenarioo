...
export class StepResource {
    ...
    get(build: BuildInfo, usecaseName: string, scenarioName: string, pageName: string, pageOccurrence: string, stepInPageOccurrence: string, labels?: string): Observable<any> {
        return this.httpClient.get<any>(`${encodeUrl(['rest', 'branch', build.branchName, 'build', build.buildName, 'usecase', usecaseName, 'scenario', scenarioName, 'pageName', pageName, 'pageOccurrence', pageOccurrence, 'stepInPageOccurrence', stepInPageOccurrence])}`,
            {
                params: {labels},
            });
    }
