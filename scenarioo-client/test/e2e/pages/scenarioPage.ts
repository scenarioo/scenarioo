...
class ScenarioPage {
    ...
    private stepView = $('div.step-overview');
    ...
    async assertFirstChangedPageDiffIconHasValue() {
        return expect($('.step-overview div:first-child .sc-step-in-overview .sc-scenario-page-title .diff-info-wrapper span').getText()).toContain('%');
    }

    async assertFirstChangedStepDiffIconHasValue() {
        return expect($('.step-overview div:first-child .sc-step-in-overview:first-child .step-title span').getText()).toContain('%');
    }
