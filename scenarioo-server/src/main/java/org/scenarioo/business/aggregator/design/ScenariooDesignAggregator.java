...
package org.scenarioo.business.aggregator.design;
...
import org.scenarioo.business.aggregator.ObjectRepository;
...
/**
 * The aggregator reads the input design files of a build and generates the aggregated design files with additional
 * precalculated data (like indexes etc.).
 *
 * Make sure to adjust the value of {@link ScenariooDesignAggregator#CURRENT_FILE_FORMAT_VERSION} when the format of
 ...
 */
public class ScenariooDesignAggregator {

	/**
	 * Version of the file format in file system. The data aggregator checks whether the file format is the same,
	 * otherwise the data has to be recalculated.
	 */
	public static final String CURRENT_FILE_FORMAT_VERSION = "2.0.1.dev1"; // TODO: Version name change?

	private final static Logger LOGGER = Logger.getLogger(ScenariooDesignAggregator.class);
	...
	// public static ScenariooDesignAggregator INSTANCE = new ScenariooDesignAggregator();

	public ScenariooDesignAggregator(final BuildImportSummary buildSummary) {
	...
	public void calculateAggregatedDataForBranch() {
	...
	private UseCaseScenariosList calculateUseCaseScenariosList() {
		...
		List<UseCase> usecases = reader.loadUsecases(getBuildIdentifier().getBranchName(), getBuildIdentifier()
				.getBuildName());
	...
	private ScenarioPageSteps calculateAggregatedDataForSteps(final UseCase usecase, final Scenario scenario,
		...
		List<Step> steps = reader.loadSteps(getBuildIdentifier().getBranchName(), getBuildIdentifier().getBuildName(),
